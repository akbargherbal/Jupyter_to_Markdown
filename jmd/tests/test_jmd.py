...
from jmd import format_markdown_cell, format_code_cell, format_output, convert_notebook
...
def markdown_cell_fixture():
    return {"cell_type": "markdown", "source": ["# Hello\n", "This is a test."]}
    ...
    notebook_content = {
        "cells": [
            {"cell_type": "markdown", "source": ["Markdown cell"]},
            {
                ...
                "outputs": [
                    {"output_type": "stream", "name": "stdout", "text": ["Code cell\n"]}
                ],
            },
        ],
        "nbformat": 4,
...
def test_cli_success(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "jmd.py", str(notebook_path)], capture_output=True, text=True
    )

    assert result.returncode == 0
    assert "[OK] Conversion successful!" in result.stdout
    assert "Output saved to:" in result.stdout


def test_cli_file_not_found():
    result = subprocess.run(
        [sys.executable, "jmd.py", "non_existent_file.ipynb"],
        capture_output=True,
        text=True,
    )
...
def test_cli_corrupted_file(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "jmd.py", str(corrupted_file)], capture_output=True, text=True
    )
...
def execute_result_fixture():
    return {"output_type": "execute_result", "data": {"text/plain": ["42"]}}


@pytest.fixture
def display_data_fixture():
    return {"output_type": "display_data", "data": {"text/plain": ["<DataFrame>"]}}
...
def test_main_function_success(mocker, tmp_path):
    ...
    mock_print.assert_any_call("[OK] Conversion successful!")
...
def test_main_function_file_not_found(mocker):
    ...
    mock_print.assert_called_with(
        "[ERROR] Error: Input file not found at 'non_existent_file.ipynb'",
        file=sys.stderr,
    )
